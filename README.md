...
Let's say you created a `.dev.env` file with the following contents:
...
1. If you run `NX_TEST=1 nx multi-env-run ...` the `NX_TEST=1` variable is not changed by `nx-multi-env` (value will be `NX_TEST=1`).
...
Serve the app in your default configuration with `prod` env chain:
```
nx multi-env-run hello-world serve --envChain=prod
```

Print the environment of the previous command added by `nx-multi-env`
```
nx multi-env-print hello-world --envChain=prod
```

Build the production version of the app with `prod` env chain and adding 2 extra env files on top:
```
nx multi-env-run hello-world build --envChain=prod --extraEnvFiles=.custom.env,apps/hello-world/.test-user.env --production
```
Print full environment of the previous command
```
nx multi-env-print hello-world --envChain=prod --extraEnvFiles=.custom.env,apps/hello-world/.test-user.env --printFullEnv
```
...
```
NX_EXAMPLE=no nx multi-env-print hello-world --envChain=dev
```
...
```
NX_EXAMPLE=no nx multi-env-print hello-world --envChain=dev --printFullEnv
```
